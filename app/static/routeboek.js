/* Routeboek-pagina: routes van de clubpagina kiezen en verwerken */
(function () {
  "use strict";
  ...
  async function loadRoutes(forceRefresh) {
    ...
    try {
      allRoutes = await api(`/api/routeboek/routes${forceRefresh ? "?refresh=1" : ""}`);
      list.innerHTML = "";
      el("route-count").textContent = `(${allRoutes.length})`;
      if (!allRoutes.length) {
        list.innerHTML = "<li class='muted'>Geen routes gevonden.</li>";
        return;
      }
      allRoutes.forEach((route) => {
        const li = document.createElement("li");
        li.dataset.name = route.name.toLowerCase();
        const elevation = route.elevation_m === null || route.elevation_m === undefined
          ? ""
          : ` · ↑ ${route.elevation_m} m`;
        const distance = route.distance_km === null || route.distance_km === undefined
          ? "onbekende afstand"
          : `${route.distance_km.toFixed(1)} km`;
        li.innerHTML =
          `<label><input type="checkbox" value="${esc(route.id)}">` +
          `<span><strong>${esc(route.name)}</strong>` +
          `<small>${distance}${elevation}</small></span></label>`;
        list.appendChild(li);
      });
    } catch (err) {
      list.innerHTML = "";
  ...
  el("process-btn").addEventListener("click", async () => {
    ...
    try {
      const items = await api("/api/routeboek/process", {
        method: "POST",
